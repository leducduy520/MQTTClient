...
DefaultActionListener::DefaultActionListener(MqttClient *parent)
    : parent_(parent) {}

void DefaultActionListener::on_failure(const mqtt::token &tok) {
  if (parent_) {
    parent_->self_handle_callback_event(
        CallbackEvent::ACTION_FAILURE,
        mqtt::token::create(tok.get_type(), *tok.get_client(), tok.get_topics(),
...
void DefaultActionListener::on_success(const mqtt::token &tok) {
  if (parent_) {
    parent_->self_handle_callback_event(
        CallbackEvent::ACTION_SUCCESS,
        token::create(tok.get_type(), *tok.get_client(), tok.get_topics(),
                      tok.get_user_context(), *tok.get_action_callback()));
  }
}
...
void MqttClient::self_handle_callback_event(CallbackEvent event,
  ...
  case CallbackEvent::DISCONNECTED: {
    ...
    if (!disconnData.props.empty()) {
      oss << " ReasonString: "
          << get<string>(disconnData.props, property::REASON_STRING)
  ...
  case CallbackEvent::ACTION_SUCCESS: {
    ...
    if (ptok) {
      ...
      if (ptok->get_type() == mqtt::token::DISCONNECT) {
        properties props;
        props.add({property::REASON_STRING,
                   "User has manually disconnected to brocker"});
        this->self_handle_callback_event(
...
MqttClient::MqttClient(const std::string &serverAddress,
    ...
    : pubListener_(new DefaultActionListener(this)),
      subListener_(new DefaultActionListener(this)),
      connListener_(new DefaultActionListener(this)),
      disconnListener_(new DefaultActionListener(this)),
...
MqttClient::MqttClient(const std::string &serverAddress,
    ...
    : connOpts_(connectOptions), pubListener_(new DefaultActionListener(this)),
      ...
      connListener_(new DefaultActionListener(this)),
      disconnListener_(new DefaultActionListener(this)),
      client_(serverAddress, clientId, createOptions), consumeFlag_(false)

{
  set_default_handler();
}

MqttClient::~MqttClient() { consume_message(false); }
...
void MqttClient::set_exception_trace(exception_trace_ptr ptr) { excPtr_ = ptr; }

void MqttClient::set_event_handler(
...
bool MqttClient::subscribe(mqtt::token_ptr &token, const std::string &topic,
                           unsigned int qos) {
  std::function<void()> fn = [this, &token, &topic, &qos]() mutable {
    dinfo1("[MqttClient] Subscribing to '")
        << topic << "' with QOS=" << qos << "..." << std::endl;
    token = client_.subscribe(topic, qos, nullptr, *subListener_);
  };
  return common_try(fn, "Subscribe");
}

bool MqttClient::subscribe(const std::string &topic, unsigned int qos,
                           bool wait, unsigned int wait_for) {
  mqtt::token_ptr token;
...
bool MqttClient::publish(mqtt::token_ptr &token, const std::string &topic,
                         const std::string &payload, unsigned int qos) {
  std::function<void()> fn = [this, &token, &topic, &qos, &payload]() mutable {
    dinfo1("[MqttClient] Publishing to '")
...
bool MqttClient::publish(const std::string &topic, const std::string &payload,
                         unsigned int qos, bool wait, unsigned int wait_for) {
  mqtt::token_ptr token;
...
bool MqttClient::connected() const { return client_.is_connected(); }

bool MqttClient::start_consume() { return this->consume_message(true); }

bool MqttClient::stop_consume() { return this->consume_message(false); }
...
bool mqttcpp::MqttClient::is_consuming() const { return consumeFlag_.load(); }
} // namespace mqttcpp
