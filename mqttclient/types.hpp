     ...
     */
        void copyFrom(const ExceptionTrace& other)
        {
            ...
            case ExceptionType::MQTT:
                ...
                break;
            case ExceptionType::STANDARD:
                new (&data_.standardException) std::exception(other.data_.standardException);
                break;
            case ExceptionType::UNKNOWN:
 ...
 */
    ...
    public:
         ...
         */
        CallbackVariant(mqtt::const_message_ptr mes_ptr) : type_(VariantType::MessagePointer)
        {
         ...
         */
        CallbackVariant(mqtt::delivery_token_ptr del_tok_ptr) : type_(VariantType::DeliveryTokenPointer)
        {
         ...
         */
        CallbackVariant(const disconnect_data& disconn_data) : type_(VariantType::DisconnectData)
        {
        ...
        mqtt::token_ptr asToken() const
        {
            if (type_ != VariantType::TokenPointer)
                throw std::runtime_error("CallbackVariant is not holding a token pointer");
        ...
        mqtt::const_message_ptr asMessage() const
        {
            if (type_ != VariantType::MessagePointer)
                throw std::runtime_error("CallbackVariant is not holding a mqtt::const_message_ptr");
        ...
        mqtt::delivery_token_ptr asDeliveryToken() const
        {
            if (type_ != VariantType::DeliveryTokenPointer)
                throw std::runtime_error("CallbackVariant is not holding a mqtt::delivery_token_ptr");
        ...
        const disconnect_data& asDisconnectData() const
        {
            if (type_ != VariantType::DisconnectData)
                throw std::runtime_error("CallbackVariant is not holding a disconnect_data");
    ...
    private:
        ...
        void copyFrom(const CallbackVariant& other)
        {
            ...
            case VariantType::DeliveryTokenPointer:
                new (&data_.del_tok_ptr) mqtt::delivery_token_ptr(other.data_.del_tok_ptr);
                break;
        ...
        union Data
        {
            ...
            ~Data()
            {} ///< Destruction is managed manually.
        } data_;
    };
} // namespace mqttcpp
